...
const typeNormalSlip = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      if (values.dob) {
        ...
        values.dob = dobDate.toLocaleDateString('en-US');
      }
      if (values.passport_issue_date) {
        const dobDate = new Date(values.passport_issue_date);
        values.passport_issue_date = dobDate.toLocaleDateString('en-US');
      }
      
      const response = await fetch('/api/create-normal', {
        method: 'POST',
        headers: {
      ...
      if (data.status === 'success') {
        toast.success('User Normal Slip Created Successfully!');
        router.push('/normal-slip');
      }
  ...
  useEffect(() => {
    // Filter only MEDSCAPE LAB medical centers
    const medscapeLabs = normalUserSlip.filter(item => 
      item.name.toLowerCase().includes('medscape lab')
    );
    
    const formattedData = medscapeLabs.map(({ name, ksa_price, kuet_price }) => ({
      value: name,
      label: `${name} - ${slipPrice !== "KW" ? ksa_price : kuet_price} BDT`,
    }));
          ...
          <div className="flex flex-col items-center justify-between border-b border-dashed border-gray-200 pb-6 dark:border-gray-700 md:flex-row">
            <div className="text-center md:text-left">
              <h2 className="mb-2 text-2xl font-bold text-gray-900 dark:text-white md:text-3xl">
                Normal Slip Application
            </h2>
              <p className="text-gray-600 dark:text-gray-400">
                Complete the form below to create a new normal slip
              </p>
            </div>
            <div className="mt-4 md:mt-0">
              <div className="inline-flex items-center rounded-lg bg-gradient-to-r from-blue-400 to-blue-500 px-6 py-3 text-white shadow-lg">
      ...
      <div className="mx-auto max-w-7xl">
        <Formik
          initialValues={{
            ...
            reference: '',
             applied_position: '76',
             tcountry: 'SA', // Default to Saudi Arabia
             city: '80', // Default to Dhaka
             visa_type: 'wv',
             medicals: '',
          }}
            ...
            <Form className="space-y-8">
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <Field
                      ...
                      value="80"
                      disabled
                      className="w-full rounded-lg border border-gray-300 bg-gray-100 px-4 py-3 text-gray-900 cursor-not-allowed dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400"
                    >
                      <option value="80">Dhaka</option>
                    </Field>
                    <p className="text-xs text-gray-500">City is fixed to Dhaka for Normal Slip</p>
                  </div>

                  {/* Country Travelling To - Saudi Arabia or Kuwait */}
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    ...
                    <p className="text-xs text-gray-500">Select between Saudi Arabia or Kuwait for Normal Slip</p>
                  </div>
                  ...
                  {/* Medical Center - Only MEDSCAPE LAB */}
                  <div className="space-y-2 md:col-span-2">
                    ...
                    <Select
                      ...
                      isMulti={false}
                      isSearchable={true}
                      placeholder="Search and select Medical Center (MEDSCAPE LAB only)"
                    ...
                    />
                    <p className="text-xs text-gray-500">Only MEDSCAPE LAB medical centers are available for Normal Slip</p>
                    <ErrorMessage
                      name="medicals"
                      component="div"
                      className="text-sm text-red-500"
              ...
              <div className="flex justify-center">
                <Button 
                  ...
                  className="group relative w-full max-w-md transform rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 px-8 py-4 text-lg font-semibold text-white shadow-lg transition-all duration-200 hover:from-blue-600 hover:to-blue-700 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
